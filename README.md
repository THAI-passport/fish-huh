...
A 2D pixel fishing game that runs in your browser. 127 species across eight
...
24 of the species are generated by [FishGen](#), a companion service that
produces pixel fish from seeds. Their sprites and stats are baked into
`src/data/genfish-pack.json` and committed, so the game has **no runtime
dependency** on it — the service only needs to run if you want to regenerate the
pack.
